...
class MarkdownExporter:
    ...
    def export(self, output_path: str) -> str:
        md_content = self.build()
        self.write(output_path, md_content)
        return md_content

    def build(self) -> str:
        """MD 문자열을 만든다. 파일에는 쓰지 않는다."""
        return self._build_markdown()

    @staticmethod
    def write(output_path: str, md_content: str) -> None:
        """만든 MD 문자열을 파일에 쓴다."""
    ...
    def _render_block(
        ...
        # Always render child blocks in order, whether or not the parent has text.
        # A parent's text_block.text holds only its own text, not its children's,
        # so skipping children here would drop their text from the output.
        # (A footer with its own text already returned above, dropping its subtree.)
...
def prepend_warning(md_content: str, title: str, items: list[str]) -> str:
    """MD 본문 앞에 경고 블록을 붙인다. 본문은 한 글자도 바꾸지 않는다."""
    lines = [f"> **{title}**", ">"]
