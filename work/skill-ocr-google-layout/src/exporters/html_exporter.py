...
class HTMLExporter:
    ...
    def export(self, output_path: str) -> None:
        html_content, _ = self.build(output_path)
        self.write(output_path, html_content)

    def build(self, output_path: str) -> tuple[str, dict[int, list]]:
        """HTML 문자열과 그 HTML 을 만든 페이지 배정 결과를 돌려준다.

        HTML 파일은 쓰지 않는다. 이미지를 넣지 않는 모드면 페이지 PNG 는 지금처럼
        ``<base>_images/`` 에 쓴다. 배정 결과는 ``_group_blocks_by_page()`` 모양이다
        (``{page: [PageItem(block, mode, path), ...]}``).
        """
        ...
        return self._build_html(page_blocks), page_blocks

    @staticmethod
    def write(output_path: str, html_content: str) -> None:
        """만든 HTML 문자열을 파일에 쓴다."""
    ...
    def _build_html(self, page_blocks: dict[int, list] | None = None) -> str:
        self.headings = []
        self.heading_counter = 0
    ...
    def _group_blocks_by_page(self) -> dict[int, list]:
        ...
        Returns:
            page_blocks[page_num] = [PageItem(block, render_mode, path), ...]
            render_mode: 'full' (render with children) or 'text_only' (own text only)
            path: the block's position path (see ``iter_blocks_with_paths``)
        """
        page_blocks: dict[int, list] = {}
        if not self.doc.document_layout:
            return page_blocks

        for index, root in enumerate(self.doc.document_layout.blocks):
            covered: BlockPath | None = None
            for path, block in iter_blocks_with_paths(root, (index,)):
                if covered is not None and path[: len(covered)] == covered:
                    continue  # already rendered as part of a "full" ancestor
                covered = path if self._distribute_block(block, path, page_blocks) else None
    ...
    def _distribute_block(
        ...
        """Place one block on a page. Returns True when its whole subtree is placed.

        Blocks are visited in document order; when this returns False the
        caller goes on to the block's children, which get the same rule.
        Single- vs multi-page is decided by the page range of the block's whole
        subtree (itself plus all descendants), not its own page_span alone.
        - No page_span anywhere in the subtree: whole block on page 1.
        - Subtree on one page: whole block ("full") on that page.
        - Subtree across pages: a text block puts its own text (if any) on its
          own start page (subtree start page if it has no page_span) as
          "text_only", and each child is distributed by the same rule. Tables
          and lists stay whole ("full") on their start page.
        """
        subtree_range = subtree_page_range(block)
        if subtree_range is None:
            page_blocks.setdefault(1, []).append(PageItem(block, "full", path))
            return True

        subtree_start, subtree_end = subtree_range
        if subtree_start == subtree_end:
            page_blocks.setdefault(subtree_start, []).append(
                PageItem(block, "full", path)
            )
            return True

        own_range = block_page_range(block)
        page_start = own_range[0] if own_range else subtree_start

        if block.text_block:
            if block.text_block.text and block.text_block.text.strip():
                page_blocks.setdefault(page_start, []).append(
                    PageItem(block, "text_only", path)
                )
            return False
        if block.list_block or block.table_block:
            page_blocks.setdefault(page_start, []).append(
                PageItem(block, "full", path)
            )
            return True
...
_CONTENT_AREA_OPEN = '<main class="content-area" id="content-area">'

_WARNING_STYLE = (
    "background:#fdecea;border:2px solid #c0392b;color:#7b1d13;"
    "padding:12px 16px;margin-bottom:14px;font-size:14px;line-height:1.6;"
    "flex-shrink:0"
)


def add_warning_banner(html_content: str, title: str, items: list[str]) -> str:
    """본문 영역 맨 위에 경고 배너를 넣는다. 나머지 HTML 은 한 글자도 바꾸지 않는다.

    배너는 본문 칸(``text-col``) 밖에 있어 본문 텍스트로 읽히지 않는다.
    """
    if _CONTENT_AREA_OPEN not in html_content:
        raise ValueError("HTML 에서 본문 영역 시작 태그를 찾지 못했다")
    lines = [
        f'<div class="verify-warning" role="alert" style="{_WARNING_STYLE}">',
        f"<p style=\"margin:0 0 6px;font-weight:700\">{_html_escape(title)}</p>",
        '<ul style="margin:0;padding-left:20px">',
        *(f"<li>{_html_escape(item)}</li>" for item in items),
        "</ul>",
        "</div>",
    ]
    return html_content.replace(
        _CONTENT_AREA_OPEN, _CONTENT_AREA_OPEN + "\n" + "".join(lines), 1
    )
