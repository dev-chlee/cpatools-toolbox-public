import os
import platform
import sys

# Workaround: Python 3.14 + Windows bug where platform.uname() hangs on WMI query.
# Libraries like aiohttp call platform.system()/machine() at import time,
# so this must be placed before all other imports.
if sys.platform == "win32" and hasattr(platform, "_uname_cache") and platform._uname_cache is None:
    try:
        platform._uname_cache = platform.uname_result(
            "Windows",
            os.environ.get("COMPUTERNAME", ""),
            "10",
            platform._syscmd_ver()[2] or "10.0",
            os.environ.get("PROCESSOR_ARCHITECTURE", "AMD64"),
            os.environ.get("PROCESSOR_IDENTIFIER", ""),
        )
    except Exception:
        pass

import argparse
import json
import time
from pathlib import Path

import fitz

from src import cpa_storage
from src.batch_processor import BatchProcessor
from src.config import DocumentAIConfig
from src.exporters.html_exporter import HTMLExporter, add_warning_banner
from src.exporters.markdown_exporter import MarkdownExporter, prepend_warning
from src.exporters.verify import VerifyResult, verify_outputs
from src.logger import fmt_size, log_timer, setup_logging
from src.processor import process_document, process_document_parallel

# 변환 결과 검증 실패가 하나라도 있으면 이 코드로 끝난다(scripts/run_inbox_batch.py 와 약속).
EXIT_VERIFY_FAILED = 3
SKILL = "skill-ocr-google-layout"


def default_output_dir() -> str:
    """결과 폴더 기본값: 보관소 output."""
    return str(cpa_storage.slot(SKILL, "output"))


def main():
    parser = argparse.ArgumentParser(
        description="GCP Document AI Layout Parser OCR"
    )
    parser.add_argument(
        "--file", "-f", nargs="+", help="PDF file path(s) (multiple allowed)"
    )
    parser.add_argument("--dir", "-d", help="PDF directory path (process all PDFs in folder)")
    parser.add_argument("--gcs", "-g", help="GCS URI (gs://bucket/path)")
    parser.add_argument(
        "--batch", "-b", help="Batch processing: GCS input prefix (gs://bucket/folder/)"
    )
    parser.add_argument(
        "--batch-output", help="Batch processing: GCS output prefix"
    )
    parser.add_argument(
        "--output", "-o", help="결과 폴더 (기본: 보관소 output)"
    )
    parser.add_argument(
        "--format",
        choices=["html", "md", "both"],
        default="both",
        help="Output format (default: both)",
    )
    parser.add_argument(
        "--embed-images",
        action="store_true",
        help="Embed images as base64 in HTML (default: separate files)",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=None,
        help="Override Document AI chunking token size (default: .env CHUNK_SIZE)",
    )
    parser.add_argument(
        "--cache",
        default=None,
        help="API response cache file path (load if exists, save otherwise)",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
        default=4,
        help="Parallel API call workers for large PDFs (default: 4, max: 10)",
    )
    parser.add_argument(
        "--no-parallel",
        action="store_true",
        help="Disable parallel chunked processing (force GCS batch for large PDFs)",
    )
    parser.add_argument(
        "--save-response",
        action="store_true",
        help=(
            "검증 결과와 관계없이 파일마다 원응답을 <base>_response.json 으로 결과 폴더에 "
            "남긴다 (기본 꺼짐; 검증 실패 시에는 항상 남김). --cache 로 다시 읽을 수 있다"
        ),
    )
    args = parser.parse_args()
    try:
        args.output = str(cpa_storage.guard(args.output)) if args.output else default_output_dir()
        if args.cache and not Path(args.cache).exists():
            cpa_storage.guard(args.cache)  # 없으면 여기에 원응답을 저장한다
    except cpa_storage.StorageError as exc:
        parser.error(str(exc))

    config = DocumentAIConfig.from_env()
    logger = setup_logging(args.output)

    # Override chunk-size from CLI
    if args.chunk_size is not None:
        config.processing.chunk_size = args.chunk_size
    args.max_workers = max(1, min(args.max_workers, 10))
    Path(args.output).mkdir(parents=True, exist_ok=True)

    results: list[VerifyResult] = []
    if args.batch:
        _run_batch(config, args, logger)
    elif args.file or args.dir or args.gcs:
        # Collect files: --file + --dir -> pdf_files list
        try:
            pdf_files = _collect_pdf_files(args)
        except (FileNotFoundError, NotADirectoryError, ValueError) as e:
            parser.error(str(e))

        if not pdf_files and args.gcs:
            results.append(_run_single_gcs(config, args, logger))
        elif len(pdf_files) == 1:
            results.append(_run_single_local(config, args, pdf_files[0], logger))
        elif len(pdf_files) >= 2:
            results.extend(_run_multi_local(config, args, pdf_files, logger))
        else:
            parser.error("No PDF files to process.")
    else:
        parser.error("One of --file, --dir, --gcs, or --batch must be specified.")

    failed = sum(1 for result in results if not result.success)
    if failed:
        logger.error(
            f"변환 결과 검증 실패 {failed}건 (전체 {len(results)}건): "
            f"종료 코드 {EXIT_VERIFY_FAILED}으로 끝냅니다."
        )
        sys.exit(EXIT_VERIFY_FAILED)


def _collect_pdf_files(args) -> list[str]:
    """Collect PDF file paths from --file and --dir arguments."""
    pdf_files: list[str] = []

    if args.file:
        for f in args.file:
            p = Path(f).resolve()
            if not p.exists():
                raise FileNotFoundError(f"File not found: {f}")
            pdf_files.append(str(p))

    if args.dir:
        dir_path = Path(args.dir).resolve()
        if not dir_path.is_dir():
            raise NotADirectoryError(f"Directory not found: {args.dir}")
        for p in sorted(dir_path.glob("*.pdf")):
            pdf_files.append(str(p))

    # Check for duplicate filenames (stems) to prevent output folder conflicts
    stems = [Path(f).stem for f in pdf_files]
    seen: dict[str, str] = {}
    for path, stem in zip(pdf_files, stems):
        if stem in seen:
            raise ValueError(
                f"Filename conflict: '{stem}' - {seen[stem]} vs {path}. "
                f"Filenames must be unique to avoid output folder collisions."
            )
        seen[stem] = path

    return pdf_files


def _run_single_local(
    config, args, pdf_path: str, logger, output_dir: str | None = None
) -> VerifyResult:
    """Process a single local PDF (online API, parallel chunked, or batch)."""
    file_path = Path(pdf_path)
    with open(file_path, "rb") as f:
        pdf_bytes = f.read()

    try:
        with fitz.open(stream=pdf_bytes, filetype="pdf") as pdf_doc:
            page_count = len(pdf_doc)
    except Exception as e:
        raise ValueError(
            f"PDF 를 열 수 없습니다(손상되었거나 PDF 형식이 아님): {file_path.name} — {e}"
        ) from e

    logger.info(f"Processing: {file_path.name} ({page_count} pages)")

    needs_splitting = page_count > config.max_online_pages
    use_parallel = needs_splitting and not args.no_parallel

    if use_parallel:
        # Split + parallel online API
        logger.info(
            f"Mode: parallel chunked online API - "
            f"{page_count} pages, {config.max_online_pages} pages/chunk"
        )
        start_time = time.time()
        try:
            with log_timer(logger, "Parallel API complete"):
                doc = process_document_parallel(
                    config,
                    pdf_bytes,
                    max_workers=args.max_workers,
                    cache_path=args.cache,
                )
        except Exception as e:
            if not config.gcs_bucket:
                raise
            logger.warning(f"Parallel processing failed: {e}")
            logger.info("Falling back to GCS batch processing...")
            with log_timer(logger, "Batch API fallback complete"):
                doc = _run_batch_fallback(config, pdf_path, logger)
        total_time = time.time() - start_time
    elif needs_splitting:
        # --no-parallel or fallback: GCS batch
        if not config.gcs_bucket:
            raise ValueError(
                f"PDF has {page_count} pages, exceeding the online processing limit "
                f"({config.max_online_pages} pages). "
                f"Set GCS_BUCKET in .env for batch processing, "
                f"or remove --no-parallel to use parallel chunked processing."
            )
        logger.info(
            f"Mode: batch (GCS) - {page_count} pages > {config.max_online_pages}"
        )
        start_time = time.time()
        with log_timer(logger, "Batch API complete"):
            doc = _run_batch_fallback(config, pdf_path, logger)
        total_time = time.time() - start_time
    else:
        logger.info("Mode: online API")
        start_time = time.time()
        with log_timer(logger, "Online API complete"):
            doc = process_document(
                config,
                cache_path=args.cache,
                raw_content=pdf_bytes,
            )
        total_time = time.time() - start_time

    base_name = file_path.stem
    out = output_dir or args.output
    result = _export(doc, pdf_bytes, base_name, out, args, logger)

    logger.info(f"Total time: {total_time:.1f}s")
    return result


def _run_batch_fallback(config, pdf_path: str, logger):
    """Run GCS batch processing for a single file and return the Document."""
    processor = BatchProcessor(config)
    results = processor.process_local_files([pdf_path])
    return next(iter(results.values()))


def _run_single_gcs(config, args, logger) -> VerifyResult:
    """Process a single file via GCS URI."""
    logger.info(f"Processing: {args.gcs}")
    logger.info("Mode: online API (GCS)")

    start_time = time.time()
    with log_timer(logger, "Online API complete"):
        doc = process_document(
            config,
            gcs_uri=args.gcs,
            cache_path=args.cache,
        )
    total_time = time.time() - start_time

    # Use string split instead of Path().stem for GCS URIs (Windows compat)
    base_name = args.gcs.rstrip("/").rsplit("/", 1)[-1].rsplit(".", 1)[0]
    result = _export(doc, None, base_name, args.output, args, logger)

    logger.info(f"Total time: {total_time:.1f}s")
    return result


def _run_multi_local(config, args, pdf_files: list[str], logger) -> list[VerifyResult]:
    """Process multiple local PDFs individually (online/parallel per file).

    A verification failure in one file does not stop the others; the caller
    decides the exit code from the returned results.
    """
    file_names = [Path(f).name for f in pdf_files]
    logger.info(f"Processing: {len(pdf_files)} files ({', '.join(file_names)})")

    start_time = time.time()
    results: list[tuple[str, str, VerifyResult]] = []  # (base_name, output_dir, result)

    for i, pdf_path in enumerate(pdf_files, 1):
        base_name = Path(pdf_path).stem
        file_output_dir = str(Path(args.output) / base_name)
        Path(file_output_dir).mkdir(parents=True, exist_ok=True)

        logger.info(f"─── [{i}/{len(pdf_files)}] {Path(pdf_path).name} ───")
        result = _run_single_local(config, args, pdf_path, logger, output_dir=file_output_dir)
        results.append((base_name, file_output_dir, result))

    total_time = time.time() - start_time

    # Processing results summary
    logger.info("─" * 40)
    logger.info("Results:")
    for base_name, file_output_dir, result in results:
        sizes = _get_output_sizes(Path(file_output_dir), base_name)
        status = "" if result.success else " [검증 실패]"
        logger.info(f"  {base_name} → {file_output_dir}/ ({sizes}){status}")
    logger.info(f"Total time ({len(pdf_files)} files): {total_time:.1f}s")
    return [result for _, _, result in results]


def _export(doc, pdf_bytes, base_name, output_dir, args, logger) -> VerifyResult:
    """Build HTML/MD, verify them against the response, then write them.

    Verification runs on the built strings before any warning is added, and
    only the formats actually built are checked. On failure the outputs are
    still written, each with a warning at the top, together with
    ``<base>_verify_failed.json`` and ``<base>_response.json``.
    """
    html_path = f"{output_dir}/{base_name}.html"
    md_path = f"{output_dir}/{base_name}.md"
    failed_path = Path(output_dir) / f"{base_name}_verify_failed.json"
    response_path = Path(output_dir) / f"{base_name}_response.json"

    html_content = page_assignment = markdown = None
    if args.format in ("html", "both"):
        html_exporter = HTMLExporter(doc, pdf_bytes, embed_images=args.embed_images)
        html_content, page_assignment = html_exporter.build(html_path)
    if args.format in ("md", "both"):
        markdown = MarkdownExporter(doc).build()

    result = verify_outputs(
        doc, markdown=markdown, html=html_content, page_assignment=page_assignment
    )

    if not result.success:
        title, items = _warning_text(result, failed_path.name, response_path.name)
        if html_content is not None:
            html_content = add_warning_banner(html_content, title, items)
        if markdown is not None:
            markdown = prepend_warning(markdown, title, items)

    if html_content is not None:
        HTMLExporter.write(html_path, html_content)
        logger.info(f"HTML saved: {html_path}")
        if not args.embed_images and pdf_bytes:
            logger.info(f"Images saved: {output_dir}/{base_name}_images/")
    if markdown is not None:
        MarkdownExporter.write(md_path, markdown)
        logger.info(f"Markdown saved: {md_path}")

    if result.success:
        logger.info(f"변환 결과 검증 [{base_name}]: {result.summary}")
    else:
        failed_path.write_text(
            json.dumps(result.to_dict(), ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        logger.error(f"변환 결과 검증 [{base_name}]: {result.summary}")
        logger.error(f"  누락 목록: {failed_path}")

    if not result.success or args.save_response:
        _save_response(doc, response_path)
        logger.info(
            f"원응답 저장: {response_path} "
            f"(--cache 로 지정하면 API 호출 없이 다시 변환할 수 있음)"
        )

    return result


def _warning_text(
    result: VerifyResult, failed_name: str, response_name: str
) -> tuple[str, list[str]]:
    """MD 경고 블록과 HTML 경고 배너에 같은 내용을 쓰도록 제목과 항목을 만든다."""
    counts: list[str] = []
    if result.text_none:
        counts.append("텍스트 없음(원응답에서 비교할 텍스트 블록을 찾지 못함)")
    if "md" in result.checked_formats:
        counts.append(f"MD 누락 {len(result.md_missing)}개")
    if "html" in result.checked_formats:
        counts.append(f"HTML 누락 {len(result.html_missing)}개")
        counts.append(f"HTML 페이지 오배치 {len(result.html_misplaced)}개")
    if result.checked_formats:
        counts.append(f"표 구조 불일치 {len(result.table_mismatches)}개")
        counts.append(f"표 구조 확인 불가 {len(result.table_unverifiable)}개")
    title = (
        "[검증 실패] 원응답과 대조한 결과 누락, 오배치 또는 표 구조 문제가 있습니다. "
        "이 파일은 완전한 변환본이 아닙니다."
    )
    items = [
        f"실패 사유별 건수: {', '.join(counts)}",
        f"상세 목록: {failed_name}",
        f"원응답: {response_name} (--cache 로 지정하면 API 호출 없이 다시 변환할 수 있음)",
    ]
    return title, items


def _save_response(doc, response_path: Path) -> None:
    """메모리의 Document 를 --cache 와 같은 JSON 형식으로 쓴다."""
    response_path.parent.mkdir(parents=True, exist_ok=True)
    response_path.write_text(type(doc).to_json(doc), encoding="utf-8")


def _get_output_sizes(output_dir: Path, base_name: str) -> str:
    """Format output file sizes."""
    parts = []
    html_path = output_dir / f"{base_name}.html"
    md_path = output_dir / f"{base_name}.md"
    if html_path.exists():
        parts.append(f"HTML: {fmt_size(html_path.stat().st_size)}")
    if md_path.exists():
        parts.append(f"MD: {fmt_size(md_path.stat().st_size)}")
    return ", ".join(parts) if parts else "no output"


def _run_batch(config, args, logger) -> None:
    """GCS remote batch processing."""
    if not args.batch_output:
        raise ValueError("--batch-output (GCS output path) is required for batch processing.")

    processor = BatchProcessor(config)
    processor.process_batch(args.batch, args.batch_output)
    logger.info(f"Batch results saved: {args.batch_output}")


if __name__ == "__main__":
    main()
