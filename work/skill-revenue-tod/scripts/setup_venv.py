...
사용:
    python scripts/setup_venv.py            # 스킬-로컬 venv 생성 + 의존성 설치
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description=f"{skill_root.name} 런타임 가상환경 준비")
    ...
    if not py.exists():
        print(f"[오류] venv Python을 찾을 수 없습니다: {py}", file=sys.stderr)
        return 1
    ...
    if (skill_root / "requirements.lock").exists():
        ...
        print("[경고] 의존성 파일 없음 — 빈 venv만 생성")

    print(f"[완료] venv 준비: {venv_dir}")
    print(f"[완료] Python: {py}")
    return 0
