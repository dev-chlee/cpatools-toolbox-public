...
def _render(pdf, page, target, dpi):
    # Poppler 우선, 없으면 기존 pdfplumber 의존성의 PDFium 렌더러 사용.
    if shutil.which("pdftoppm"):
        ...
        with pdfplumber.open(pdf) as doc:
            doc.pages[page - 1].to_image(resolution=dpi).original.save(target)
...
def prepare(pdf, ocr, output, source_pages=None, dpi=200):
    ...
    with tempfile.TemporaryDirectory(dir=output.parent, prefix=".registry-prepare-") as tmp:
        ...
        review = {"schema": SCHEMA, "bundle_sha256": sha256(tmp / "bundle.json"),
                  ...
                  "analysis_expectations": {"owner": None, "location": None, "mortgages": None, "other_encumbrances": None, "seizures": None},
...
def validate(bundle_path, review_path):
    ...
    source_ids = {'-'.join(m) for m in re.findall(
        r'고\s*유\s*번\s*호\s*[:：]?\s*(\d{4})\s*[-－–]\s*(\d{4})\s*[-－–]\s*(\d{6})(?!\d)', source_text)}
