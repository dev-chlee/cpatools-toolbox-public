...
사용법:
    ...
    JSON 형식:
    {
        "output_path": "<보관소>/output/WP_공시지가_조회결과.xlsx",   # 필수. git 저장소 안이면 거부
        "img_dir": "<보관소>/output/images",
        "company_name": "(주)OO",
...
import argparse
import cpa_storage
from query_api import price_order_key
...
def create_workpaper(data):
    """감사 워크페이퍼를 생성한다."""
    output_path = str(cpa_storage.guard(data["output_path"]))
    img_dir = data.get("img_dir", os.path.dirname(output_path))
    # API 응답 순서나 별도 표시 문자열에 의존하지 않고 같은 행의 연도·금액·일자를 사용한다.
    props = [dict(p, data=sorted(p['data'], key=lambda r: price_order_key(r[0], r[4], r[5]),
                                 reverse=True)) for p in data["properties"]]
    notes = data.get("notes", [])
    image_paths = {}
    for p in props:
        filename = p.get('img_file')
        path = os.path.join(img_dir, filename) if filename else ''
        image_paths[id(p)] = path if path and os.path.isfile(path) else None
    if any(image_paths.values()):
        try:
            from PIL import Image as _PILImage
        except ImportError as exc:
            raise RuntimeError('이미지 삽입에는 Pillow가 필요합니다. 스킬 폴더에서 실행하세요: '
                               f'python scripts/setup_venv.py --with-images --venv "{sys.prefix}"') from exc
    ...
    set_cell(ws, r, 2, '3. 조회 결과 스크린샷',
             Font(name='맑은 고딕', size=12, bold=True, color='1A5276'))
    r += 1
    image_count = sum(bool(p) for p in image_paths.values())
    image_note = (f'조회결과 이미지 {image_count}/{len(props)}건 첨부. 미첨부 물건은 개별 시트에 표시합니다.'
                  if image_count else '조회결과 이미지 미첨부 — API 조회 데이터로 작성한 조서입니다.')
    set_cell(ws, r, 2, image_note,
             font_data, alignment=align_left)
    ...
    headers4 = ['순번', '소재지', '지번', '개별공시지가(원/㎡)', '기준일자', '공시일자', '과거 데이터(건수)']
    ...
    for ri, p in enumerate(props):
        price_val = p['data'][0][3] if p['data'] else None
        base_dt = f"{p['data'][0][0]}년 {p['data'][0][4]}" if p['data'] else '조회 결과 없음'
    ...
    for p in props:
        ...
        latest = p['data'][0] if p['data'] else None
        set_cell(ws, r, 2, f'{latest[0]}년 공시지가' if latest else '공시지가', font_label, fill_header, align_left, border_all)
        ws.merge_cells(f'C{r}:G{r}')
        set_cell(ws, r, 3, f'{latest[3]:,} 원/㎡' if latest else '조회 결과 없음',
        ...
        set_cell(ws, r, 2, '가격 기준일', font_label, fill_header, align_left, border_all)
        ws.merge_cells(f'C{r}:G{r}')
        set_cell(ws, r, 3, f'{latest[0]}년 {latest[4]} / 공시일 {latest[5]}' if latest else '조회 결과 없음',
                 font_data, None, align_left, border_all)
        for cc in range(4, 8): ws.cell(row=r, column=cc).border = border_all
        r += 1
        ...
        recent_years = sorted({int(row[0]) for row in p['data']}, reverse=True)[:6]
        recent_rows = [row for row in p['data'] if int(row[0]) in recent_years]
        for ri, row_data in enumerate(recent_rows):
            bg = fill_yellow if ri == 0 else (fill_light if ri % 2 == 0 else None)
            for ci, val in enumerate(row_data):
                al = align_right if ci == 3 else align_center
                nf = '#,##0' if ci == 3 else None
        ...
        img_path = image_paths[id(p)]
        if img_path:
            img = XlImage(img_path)
            # 실제 스크린샷은 세로로 길 수 있으므로 원본 비율을 유지한다(폭 고정, 높이 비례).
            target_w = 760
            try:
                with _PILImage.open(img_path) as _im:
                    ow, oh = _im.size
                target_h = int(target_w * oh / ow) if ow else 340
            except Exception:
                target_h = 340
            img.width = target_w
            img.height = target_h
            ws.add_image(img, f'B{r}')
            for _ in range(int(target_h / 20) + 2):
                ws.row_dimensions[r].height = 20
                r += 1
        else:
            set_cell(ws, r, 2, '이미지 미첨부 — API 조회 데이터만 수록', font_note, alignment=align_left)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description='공시지가 워크페이퍼 생성 (stdin JSON 입력)')
    ap.parse_args()
    data = json.load(sys.stdin)
    try:
        result = create_workpaper(data)
    except cpa_storage.StorageError as e:
        ap.error(str(e))
    print(f"[OK] 워크페이퍼 생성: {result}")
