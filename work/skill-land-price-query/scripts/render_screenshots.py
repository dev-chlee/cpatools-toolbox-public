...
전제:
    ...
    python -m playwright install --only-shell chromium  # 기본 headless 캡처용
...
사용(CLI):
    ...
    # (<보관소> = CPA_SKILLS_STORAGE 또는 ~/cpa-skills-data 아래 skill-land-price-query. output_dir·--out 생략 시 <보관소>/output)
"""
...
import cpa_storage  # noqa: E402  (보관소 경로 해석 — 허브 원본 사본)
...
SKILL = "skill-land-price-query"


def default_output_dir():
    """스크린샷 출력 폴더 기본값: 보관소 output. CLI 옵션 > CPA_SKILLS_STORAGE > ~/cpa-skills-data."""
    return cpa_storage.slot(SKILL, "output")
...
def capture(sido, sigungu, dong, bun1, bun2=0,
            output_path=None, jibun_type="일반",
    ...
    Args:
        ...
        bun1/bun2: 본번/부번
        output_path: 출력 PNG 경로 (생략 시 보관소 output/shot.png, git 저장소 안이면 거부)
    ...
    Raises:
        ...
        cpa_storage.StorageError: 출력 경로가 git 저장소 안
    """
    output_path = (str(cpa_storage.guard(output_path)) if output_path
                   else str(default_output_dir() / "shot.png"))
    try:
        ...
        raise ScreenshotError(
            ...
            f'    python scripts/setup_venv.py --with-capture --venv "{sys.prefix}"') from e
        ...
        try:
            ...
            if "install" in str(e).lower() or "executable" in str(e).lower():
                raise ScreenshotError(
                    ...
                    + ("    python -m playwright install --only-shell chromium" if headless else
...
def _run_cli(argv=None):
    ...
    ap.add_argument("--out", help="출력 PNG 경로 (기본: 보관소 output/shot.png)")
    ...
    if args.sido is None and not sys.stdin.isatty():
        ...
        try:
            output_dir = (str(cpa_storage.guard(data["output_dir"])) if data.get("output_dir")
    ...
    try:
        args.out = str(cpa_storage.guard(args.out)) if args.out else str(default_output_dir() / "shot.png")
    except cpa_storage.StorageError as e:
        ap.error(str(e))
