...
def price_order_key(year, base_md, notice_ymd):
    """가격기준 연도·월일·공시일 순. 같은 연도의 수시공시를 별도 행으로 보존한다."""
    md = tuple(int(v) for v in re.findall(r'\d+', str(base_md)))
    return int(year or 0), md, re.sub(r'\D', '', str(notice_ymd or ''))
...
def query_price(opener, reg, eub, bun1, bun2=0, san="1", year=""):
    ...
    rows = m.get("list") or []
    rows.sort(key=lambda x: price_order_key(
        x.get("base_year"), x.get("base_md", ""), x.get("notice_ymd", "")), reverse=True)
